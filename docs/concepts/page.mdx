...
```ts
import { page, section, text, int, list } from 'scrapegoat'
...
The returned object, `blogPostPage`, has a `resolve` method, which will fetch a blog post by ID.

```ts
const blogPost = await blogPostPage.resolve({ postId: 'some-cool-post' });
