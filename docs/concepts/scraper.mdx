...
Because writing scrapers like this can become repetitive, scrapegoat provides a number of built-in scraper factories for handling common actions.
...
```ts
import { section, text, int } from 'scrapegoat'
...
To assist this, scrapegoat has a set of scraper factories for these scenarios.
...
<table id="users">
  ...
  <tbody>
    <tr><td>Jim</td><td>29</td></tr>
    <tr><td>AJ</td><td>31</td></tr>
  </tbody>
</table>
```
...
```ts
const usersTableScraper = table('#users', [
  { name: text() },
  { age: int() }
])
```