...
To make this as simple as possible, Severus Scrape provides a wide set of **scrapers**. Scrapers are functions which take a Cheerio DOM node and return some data.
...
indexPage.resolve().then(console.log, console.error);
...
export const getIndex = () => indexPage.resolve();
export const getPost = postId => postPage.resolve({ postId });
```
